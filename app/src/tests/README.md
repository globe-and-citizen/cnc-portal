...
Auto-imported components may be registered under the filename (without the `U` prefix). Try
`findComponent({ name: 'Xxx' })` or import the stub directly and use `findComponent(UXxxStub)`.
