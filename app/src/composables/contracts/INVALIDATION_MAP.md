...
| `['bank-events-logs' \| 'expense-events-logs' \| 'cash-remuneration-events-logs' \| 'investor-events-logs' \| 'fixed-return-events-logs' \| 'safe-deposit-router-events-logs', address]` | [`useContractEventsViaLogs`](../eventsViaLogs.ts) — the transaction feeds                                                                                |                       ❌                       |
...
| `['pastElections', address]`                                                                                                                                                             | `PastBoDElectionsSection.vue`                                                                                                                            |                       ❌                       |
...
| `fundFixedReturnRepayment`                | **cross-contract** → FixedReturn `getLendingOffer` / `totalEntitlementOf` / the 3 aggregates, `balanceOf(token, bank)`, `balanceOf(token, fixedReturn)` | **B** `CreditRepayPanel.vue:269-271` invalidates the 3 aggregates ✅ · both token balances ❌ · feed ❌                |
...
> The four `executeContractWrite` call sites are the systematic hole: `executeContractWrite` is the
> framework-agnostic function — it has no `queryClient`. Only `useContractWritesV3` invalidates.
...
| `markAsRefundable` / `claimRefund` / `refundLenders` / `acceptPartialFunding` | the 3 aggregates + same-address reads (+ token balances for the refund paths)                                                                  | **A** ✅ · **B** `RoundView.vue:179-181` / `CreditRepayPanel.vue:269-271` ✅ · token balances ❌                    |
...
Covered separately in [`queries/safe.mutations.ts`](../../queries/safe.mutations.ts): each mutation
hand-invalidates `safeKeys.info` / `transactions` / `balance` / `tokenBalance`. `safeKeys.balance`
and `safeKeys.tokenBalance` deliberately mirror the wagmi key shapes (`['balance', …]`,
`['readContract', …]`) so they cross the namespace boundary — the right instinct, applied only here.
...
| `['*-events-logs']` (6 transaction feeds)                                                                                                                                                                                         | **every** value-moving write on the matching contract                                                                                                                                                  | **❌ never invalidated anywhere.** `staleTime: 30_000` + refocus only. `refetch` is exposed but only `useCNCAccounting` calls it.                                           |
...
- **Never hand-write a key namespace in a component.** Every namespace in §2 should have exactly one
  key factory exporting it, imported by both the read and the invalidation (`safeKeys` already does
...
- **A feed is part of the write's effect.** If a write emits an event the UI renders, its
  `*-events-logs` key belongs in the same `onSuccess`. A prefix predicate
