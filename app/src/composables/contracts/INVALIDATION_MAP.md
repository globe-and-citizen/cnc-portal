...
| `['pastElections', address]`                                                                                                                                                             | [`elections/history.ts`](../elections/history.ts)                                                                                                        |                                                          ❌                                                          |
...
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `receive` (native deposit)                | `['balance', bank]`, `['balance', depositor]`, `bank-events-logs`                                                                                                                   | **B** `DepositBankForm.vue` invalidates the Bank contract-balance aggregate and the Bank feed ✅ · depositor balance ❌                          |
| `transfer` (native out)                   | `['balance', bank]`, `['balance', recipient]`, `bank-events-logs`                                                                                                                   | **B** `TransferModal.vue` invalidates the Bank contract-balance aggregate and the Bank feed ✅ · recipient balance ❌                            |
| `transferToken`                           | `balanceOf(token, bank)`, `balanceOf(token, recipient)`, `bank-events-logs`                                                                                                         | **B** `TransferModal.vue` invalidates the Bank contract-balance aggregate and the Bank feed ✅ · recipient balance ❌                            |
| `depositToken` (+ ERC-20 `approve` first) | `balanceOf(token, user)`, `balanceOf(token, bank)`, `allowance(token, user, bank)`, `bank-events-logs`                                                                              | **B** `DepositBankForm.vue` invalidates the Bank contract-balance aggregate and feed ✅ · allowance via **A** on approve ✅ · user balance ❌    |
...
| `castVote`       | `getVoteCount`, grouped `getVoteCounts`, `hasVoted`, `getVoterChoice`                                | **B** `elections/invalidation.ts` refreshes only ballot reads, intentionally skipping immutable `getElection` ✅                                                   |
...
| `['*-events-logs']` (6 transaction feeds)                                                                                                                                                                                         | **every** value-moving write on the matching contract                                                                                                                                                  | ⚠️ Bank deposits/transfers and Bank-funded FixedReturn repayments refresh their feed; other paths rely on `staleTime: 30_000` + refocus.                                    |
...
1. **Most `*-events-logs` transaction-feed write paths are not invalidated.** Bank deposits and owner transfers now refresh the Bank feed,
   and Bank-funded FixedReturn repayments refresh the FixedReturn feed. Bank dividends plus the remaining Expense, Community Credit,
   Investor, Cash Remuneration, and SafeDepositRouter paths still rely on refocus past the 30 s `staleTime`.
