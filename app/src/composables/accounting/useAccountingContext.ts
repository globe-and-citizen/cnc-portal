 ...
 *
 * The parent `AccountingView.vue` resolves the team's books once with
 * {@link useCNCAccounting} and `provide()`s the result; the routed child cards
 * ({@link useAccountingContext}) `inject()` it instead of each re-fetching. When
 * a card is rendered standalone (a direct route hit, or a unit test), it falls
 * back to self-fetching from the route's `:id` param so it still works alone.
 */
