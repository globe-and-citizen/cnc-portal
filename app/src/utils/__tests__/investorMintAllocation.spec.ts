...
describe('numeric edge guards', () => {
  it('rejects non-finite amount inputs', () => {
    expect(computeIssuedAmountFromAmountInput(Number.POSITIVE_INFINITY, 'add', 0)).toBeNull()
    expect(computeIssuedAmountFromAmountInput(Number.NaN, 'add', 0)).toBeNull()
  })

  it('rejects non-finite percentage inputs in amount sync', () => {
    expect(computeAmountFromPercentageInput(Number.POSITIVE_INFINITY, 'add', 10, 50, 5)).toBe('')
    expect(computeAmountFromPercentageInput(Number.NaN, 'ending', 10, 50, 5)).toBe('')
  })

  it('rejects non-finite amount inputs in percentage sync', () => {
    expect(computePercentageFromAmountInput(Number.POSITIVE_INFINITY, 'ending', 0, 50, 0)).toBe('')
    expect(computePercentageFromAmountInput(Number.NaN, 'add', 10, 50, 5)).toBe('')
  })
})
