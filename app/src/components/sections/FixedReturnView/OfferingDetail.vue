...
import { addTerm, maturityLabel, percentOf, resolveUser } from '@/utils'
...
type Status = 'on-track' | 'overdue' | 'partial' | 'completed'
...
const isPastMaturity = computed(() => {
  ...
  const maturity = addTerm(start, props.offering.term, props.offering.termUnit)
...
function statusFor(paidRatio: number, pastMaturity: boolean): Status {
...
const partners = computed(() =>
  (lenders.value ?? []).map((p) => {
    ...
    const status = statusFor(paidRatio, isPastMaturity.value)
