  ...
  >
    ...
    <div class="mt-3.5 flex flex-col gap-2.5">
      <div class="flex justify-between text-sm">
        ...
        <span class="font-semibold">{{ formatAmount(round.raised, round.token) }}</span>
      </div>
      <div class="flex justify-between text-sm">
        <span class="text-muted">Interest · {{ round.rate }}%</span>
        <span class="text-primary font-semibold">+ {{ formatAmount(interest, round.token) }}</span>
      </div>
      <div
      ...
      >
        ...
        <span class="text-[22px] font-extrabold tracking-tight">{{
          formatAmount(total, round.token)
        }}</span>
      </div>
