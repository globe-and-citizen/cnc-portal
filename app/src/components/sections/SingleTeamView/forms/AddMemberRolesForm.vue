<template>
  ...
  <section v-if="roleCategories">
    <div v-for="(role, index) of memberRoles" :key="index" class="p-4 flex flex-col gap-2">
  ...
  <section class="flex justify-end">
    <div class="w-6 h-6 cursor-pointer" @click="removeRole">
      <MinusCircleIcon class="size-6" />
    </div>
    <div class="w-6 h-6 cursor-pointer" @click="addRole">
  ...
  <section class="modal-action justify-center">
    ...
    <button class="btn btn-active" :disabled="isAddingRole" @click="emits('close-modal')">
      Cancel
    </button>
  </section>
</template>
...
const getErrors = () => {
  return v$.value.$errors[0].$message instanceof Array ? v$.value.$errors[0].$message[0] : []
}
...
const addRole = () => {
  ...
  if (!role) memberRoles.value.push(deepClone(initRole))
}
