<template>
    <h1 class="font-bold text-2xl">Create Role Category</h1>
    <hr class="mt-4" />
    ...
    <section class="flex flex-col gap-2">
      <label class="input input-bordered flex items-center gap-2 input-md mt-4">
        ...
        <input type="text" class="grow" placeholder="Enter a role category name" v-model="roleCategory.name" />
      </label>
    ...
    <!--Roles-->
    <section class="mt-10">
      <div v-for="(input, index) in roleCategory.roles" :key="index" class="collapse collapse-plus bg-base-200 border mt-2">
        <input type="radio" name="my-accordion-3" />
        <div class="collapse-title text-xl font-medium">Role - {{ input.name? input.name: index + 1 }}</div>
    ...
    <section class="join join-vertical w-full mt-10">
        <div v-for="(input, index) in roleCategory.entitlements" :key="index" class="collapse collapse-arrow join-item border-base-300 border">
            <input type="radio" name="category-entitlement-accordion" />
            <div class="collapse-title text-l font-medium">
              Category Entitlement - {{ getEntitlementName(input.type as unknown as number) ?? index + 1 }}
            </div>
            <div class="collapse-content">
                <AddEntitlementForm 
    ...
    <div class="modal-action justify-center">
      ...
      <button class="btn btn-primary" @click="() => {console.log('roleCategory: ', roleCategory)}" v-else>Submit</button>
  ...
  const getAvailableTypes = (index: number) => {
    return computed(() => {
      ...
      return entTypes.value.filter(type => type.id === -1 || !selectedTypes.includes(type.id) || roleCategory.value.entitlements[index].type === type.id);
    })
  };
  ...
  const emits = defineEmits(['addRole', 'searchUsers'])
  defineProps<{
    //users: User[]
    isLoading: boolean
  }>()
