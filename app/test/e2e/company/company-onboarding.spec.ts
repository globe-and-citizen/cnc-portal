/* eslint-disable max-lines -- Keep the Companies creation and Officer onboarding journeys in one discoverable spec. */
...
import { json, rejectNextWalletRequest } from '../e2e-page'
import {
  breakContract,
...
const creationTest = test.extend<{ companyCreation: CompanyApi }>({
...
creationTest.describe('[US-COMPANIES-001] Company creation', { tag: '@US-COMPANIES-001' }, () => {
  creationTest.describe.configure({ mode: 'parallel' })
   ...
   */
  creationTest(
    'validates required details and creates a company with an initial member',
    async ({ page, companyCreation: api }) => {
   ...
   */
  creationTest(
    'preserves details and lets the creator remove a member before submission',
    async ({ page, companyCreation: api }) => {
   ...
   */
  creationTest(
    'shows a failed creation and retries the same details successfully',
    async ({ page, companyCreation: api }) => {
  ...
  creationTest(
    'disables submission and back navigation while creation is pending',
    async ({ page, companyCreation: api }) => {
      const creation = gate()
  ...
  creationTest(
    'opens creation from the company menu and cancels without creating a company',
    async ({ page }) => {
   ...
   */
  creationTest(
    'blocks creation when the directory supplies an invalid member address',
    async ({ page, companyCreation: api }) => {
