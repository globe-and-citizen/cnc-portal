...
4. Run `npm run lint:md`, `npm run format:md:check`, and `bash scripts/audit-doc-drift.sh` after changing agent instructions, skills, or linked guides. The format check covers Markdown changed since `origin/develop`; subproject format checks exclude Markdown. CI supplies its pull-request base SHA.
