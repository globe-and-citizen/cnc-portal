...
contract Voting is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
  ...
  function addProposal(
    ...
    address[] memory boardOfDirectors = IBoardOfDirectors(boardOfDirectorsContractAddress)
      .getBoardOfDirectors();
    if (_isElection) {
      ...
      require(
        ...
        'Board of Directors must be set before creating proposals'
      );
      require(isBoardMember(msg.sender), 'Only board members can create proposals');
    }
    ...
    if (!_isElection) {
      require(
        boardOfDirectorsContractAddress != address(0),
        'Board of Directors contract address must be set'
      );
    ...
    if (_isElection) {
      ...
      require(_winnerCount % 2 == 1, 'Number of winners must be odd');
