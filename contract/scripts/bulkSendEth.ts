...
const { recipients, amountPerRecipient, customAmounts, delayBetweenTx, gasLimit } = BULK_TRANSFER_CONFIG
...
async function bulkSendEth() {
  ...
  if (signerBalance < totalAmount) {
    console.error(`❌ Insufficient balance! Need ${formatEther(totalAmount)} ETH but have ${formatEther(signerBalance)} ETH`)
    return
  }
  ...
  for (let i = 0; i < transferPlan.length; i++) {
    ...
    try {
      console.log(`🔄 [${i + 1}/${transferPlan.length}] Sending ${formatEther(amount)} ETH to ${recipient}...`)
      ...
      if (receipt?.status === 1) {
        ...
        console.log(`💰 Recipient balance: ${formatEther(balanceBefore)} ETH → ${formatEther(balanceAfter)} ETH\n`)
