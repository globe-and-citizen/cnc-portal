...
if [ "$1" != "localhost" ] && [ "$1" != "hardhat" ] && [ "$1" != "polFork" ]; then
  ...
  npx hardhat ignition deploy ignition/modules/VestingBeaconModule.ts --network "$1"
  npx hardhat ignition deploy ignition/modules/FixedReturnBeaconModule.ts --network "$1"
fi
