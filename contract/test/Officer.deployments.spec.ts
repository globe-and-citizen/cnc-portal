...
describe('Officer Contract', function () {
	...
	it('Should deploy contracts', async function () {
		...
		deployments.push({
			contractType: 'Bank',
			initializerData: bankAccount.interface.encodeFunctionData(
				'initialize', 
				[[], owner.address]
			)
		})
		...
		deployments.push({
			...
			initializerData: expenseAccountEip712.interface.encodeFunctionData(
				'initialize',
				[owner.address, ZeroAddress, ZeroAddress]
			)
		})
		...
		deployments.push({
			...
			initializerData: investor.interface.encodeFunctionData(
				'initialize',
				['Bitcoin', 'BTC', ZeroAddress]
			)
		})

		deployments.push({
			contractType: 'Elections',
			initializerData: elections.interface.encodeFunctionData(
				'initialize',
				[owner.address]
			)
		})
		...
		officer = (await upgrades.deployProxy(Officer, [owner.address, beaconConfigs, deployments, true], {
