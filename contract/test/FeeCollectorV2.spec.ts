...
import { FeeCollectorV2, MockERC20 } from '../typechain-types'
import { SignerWithAddress } from '@nomicfoundation/hardhat-ethers/signers'

describe('FeeCollectorV2', () => {
  let feeCollector: FeeCollectorV2
  ...
  async function deployFeeCollector(configs = INITIAL_CONFIGS, tokens: string[] = []) {
    const FeeCollectorV2Factory = await ethers.getContractFactory('FeeCollectorV2')
    return (await upgrades.deployProxy(FeeCollectorV2Factory, [owner.address, configs, tokens], {
      initializer: 'initialize'
    })) as unknown as FeeCollectorV2
  }
