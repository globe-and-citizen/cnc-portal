...
describe("Tips", function () {
  ...
  describe("pushTip", function () {
    it("should emit a PushTip event when a tip is pushed", async function () {
      ...
      await expect(
        tips.connect(sender).pushTip(recipientAddress, { value: TIP_AMOUNT })
      )
    ...
    it("should revert if the tip amount is zero", async function () {
      await expect(
        tips.connect(sender).pushTip(recipientAddress)
      ).to.be.revertedWith("Must send a positive amount.");
    });

    it("should transfer the tip amount to the recipient", async function () {
      const amountPerAddress = ethers.parseEther("10");
      const member1StartingBalance = await ethers.provider.getBalance(
        member1.address
      );
      const member2StartingBalance = await ethers.provider.getBalance(
        member2.address
      );

      await tips.pushTip(recipientAddress, { value: TIP_AMOUNT });

      const member1EndingBalance = await ethers.provider.getBalance(
        member1.address
      );
      const member2EndingBalance = await ethers.provider.getBalance(
        member2.address
      );
  ...
  describe("sendTip", function () {
    it("should emit a SendTip event when a tip is sent", async function () {
      ...
      await expect(
        tips.connect(sender).sendTip(recipientAddress, { value: TIP_AMOUNT })
      )
    ...
    it("should revert if the tip amount is zero", async function () {
      await expect(
        tips.connect(sender).sendTip(recipientAddress)
      ).to.be.revertedWith("Must send a positive amount.");
    });
  ...
  describe("withdraw", function () {
    ...
    it("should withdraw earned tips and reset the balance", async function () {
      await tips
        .connect(sender)
        .sendTip(recipientAddress, { value: TIP_AMOUNT });
