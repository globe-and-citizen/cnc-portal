...
describe('Claim Controller', () => {
  describe('POST: /claim', () => {
    ...
    it('should return 400 if hoursWorked is invalid', async () => {
      const response = await request(app).post('/claim').send({ teamId: 1, hoursWorked: -5 })
      expect(response.status).toBe(400)
  ...
  describe('GET: /claim', () => {
    it('should return 400 if teamId is invalid', async () => {
      const response = await request(app).get('/claim').query({ teamId: 'abc' })
      expect(response.status).toBe(400)
    ...
    it('should return 200 and list claims based on status filter', async () => {
      ...
      const response = await request(app).get('/claim').query({ teamId: 1, status: 'pending' })
  ...
  describe('PUT: /claim/:claimId', () => {
    it('should return 400 for invalid action', async () => {
      const response = await request(app).put('/claim/1').query({ action: 'invalid' })
    ...
    it('should return 400 for missing signature', async () => {
      const response = await request(app).put('/claim/1').query({ action: 'sign' })
    ...
    it('should return 403 if caller is not the owner of the claim for withdraw action', async () => {
      ...
      const response = await request(app).put('/claim/1').query({ action: 'withdraw' })
    ...
    describe('Disable action', () => {
      it('should return 403 if claim is not signed', async () => {
        ...
        const response = await request(app).put('/claim/1').query({ action: 'disable' })
    ...
    describe('Enable action', () => {
      it('should return 403 if claim is not disabled', async () => {
        ...
        const response = await request(app).put('/claim/1').query({ action: 'enable' })
    ...
    describe('Reject action', () => {
      it('should return 403 if claim is not pending', async () => {
        ...
        const response = await request(app).put('/claim/1').query({ action: 'reject' })
