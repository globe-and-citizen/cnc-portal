...
describe('Expense Controller', () => {
  ...
  describe('POST: /expense', () => {
    it('should return 400 if required parameters are missing', async () => {
      ...
      expect(response.body.message).toContain('Missing signature, Missing data')
    })
  ...
  describe('GET: /expenses', () => {
    it('should return 400 if teamId is invalid', async () => {
      const response = await request(app).get('/expenses').query({ teamId: 'abc' })
  ...
  describe('PUT: /expense/:id', () => {
    it('should return 400 if expense ID is invalid', async () => {
      const response = await request(app).put('/expense/abc').send({ status: 'disable' })
    ...
    it('should return 400 if status is invalid', async () => {
      const response = await request(app).put('/expense/1').send({ status: 'invalidStatus' })
    ...
    it('should return 403 if caller is not the owner of the team and the status is disable', async () => {
      ...
      const response = await request(app).put('/expense/1').send({ status: 'disable' })
    ...
    it('should update the expense status', async () => {
      ...
      const response = await request(app).put('/expense/1').send({ status: 'expired' })
