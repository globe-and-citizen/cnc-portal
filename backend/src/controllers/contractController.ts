...
export const syncContracts = async (req: AuthenticatedRequest, res: Response) => {
  const callerAddress = req.address as Address
  const body = req.body as unknown as Pick<ContractBodyRequest, 'teamId'>

  const teamId = Number(body.teamId)
  if (isNaN(teamId)) return errorResponse(400, 'Missing or invalid field: teamId', res)
...
export const getContracts = async (req: Request, res: Response) => {
  ...
  if (isNaN(teamId)) return errorResponse(400, 'Invalid or missing teamId', res)
...
export const addContract = async (req: AuthenticatedRequest, res: Response) => {
  const callerAddress = req.address as Address
