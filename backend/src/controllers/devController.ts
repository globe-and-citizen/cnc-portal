...
export const generateSiweSignature = async (req: Request, res: Response) => {
  try {
    ...
    if (!nonce || !address || !domain || !chainId) {
      return errorResponse(400, "Missing required fields in messageParams: nonce, address, domain, chainId", res);
    }
    ...
    if (account.address.toLowerCase() !== address.toLowerCase()) {
      return errorResponse(400, `Address ${address} does not match the provided private key (account: ${account.address})`, res);
    }
    ...
    const siweMessage = new SiweMessage({
      ...
      statement: statement || 'I accept the MetaMask Terms of Service: https://community.metamask.io/tos',
      uri: uri || `http://${domain}`,
      version: '1',
      chainId: chainId,
      nonce: nonce,
