...
type wageBodyRequest = Pick<
  Wage,
  'teamId' | 'userAddress' | 'cashRatePerHour' | 'tokenRatePerHour' | 'maximumHoursPerWeek'
>
export const setWage = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (missingParameters.length > 0) {
    return errorResponse(400, `Missing or invalid parameters: ${missingParameters.join(', ')}`, res)
  }
  ...
  try {
    ...
    if (!(await isOwnerOfTeam(callerAddress as Address, teamId))) {
...
// /wage/?teamId=teamId
export const getWages = async (req: AuthenticatedRequest, res: Response) => {
