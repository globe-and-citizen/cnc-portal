...
La page HTML offre : filtre par contrat, filtre par statut (`ok` / `partial` / `missing` / `not-ok`), recherche par nom de fonction, et un
détail dépliable des orphelins et des usages.
...
7. **Consommateurs** : walk de `app/src/` (hors `composables/` et dossiers de tests) ; pour chaque fichier qui appelle `useXxx(`, recherche
   d'un spec adjacent + détection de `describe.skip` / `it.skip`.
...
`abitype` est résolu dynamiquement depuis `app/node_modules/abitype/dist/esm/exports/index.js`. Si absent, un formateur de signature inline
prend le relais (sans abitype).

## Orphelins détectés

Le rapport liste trois catégories d'orphelins par contrat :

- **composablesWithoutFunction** — composables qui pointent vers une fonction ABI qui n'existe pas (ex. sur Bank : `useBankSupportedTokens`
  → `supportedTokens`, qui n'est plus dans l'ABI)
