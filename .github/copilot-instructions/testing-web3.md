...
Don't mock viem — mock the wagmi layer the app actually calls. If you find yourself reaching for `viem` mocks, the production code is
probably bypassing wagmi's abstraction.
...
- **User-rejected** (wallet popup dismissed) — wagmi throws `UserRejectedRequestError`. Map to a non-error toast (info / silent), never a
  red error toast.
