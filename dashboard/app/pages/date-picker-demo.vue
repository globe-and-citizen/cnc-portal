...
/**
 * Dev playground for {@link DatePicker} (issue #2051).
 *
...
<template>
  <div class="mx-auto max-w-2xl space-y-10 p-8">
    ...
    <section class="space-y-3">
      ...
      <DatePicker v-model="asOf" mode="date" />
    ...
    <section class="space-y-3">
      ...
      <DatePicker v-model="period" mode="range" />
      <dl class="grid grid-cols-[8rem_1fr] gap-y-1 text-sm">
        <dt class="text-muted">
          start
        </dt>
    ...
    <section class="space-y-2 rounded-md bg-elevated/50 p-4 text-sm">
      ...
      <pre class="overflow-x-auto text-xs text-muted"><code>&lt;DatePicker v-model="asOfDate" mode="date" /&gt;
&lt;DatePicker v-model="period" mode="range" /&gt;</code></pre>
    </section>
  </div>
</template>
