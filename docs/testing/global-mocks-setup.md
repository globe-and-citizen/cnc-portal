...
The CNC Portal uses a centralized mocking system that provides consistent, reusable mocks for all query and mutation
hooks. This ensures:
...
| `wage.queries.ts`         | `useTeamWagesQuery`, `useSetMemberWageQuery`                                                      | Wage configuration           |
...
it("should invalidate team queries after update", async () => {
  ...
  vi.mocked(useUpdateTeamQuery).mockReturnValue(mockMutation);
