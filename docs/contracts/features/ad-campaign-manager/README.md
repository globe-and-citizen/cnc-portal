...
**Epic Goal:** Let advertisers fund on-chain ad campaigns and allow admins to claim per-click/impression payments
against those budgets. **Contract File:** `contracts/AdCampaignManager.sol` **Upgradeable:** No (standard Ownable)
...
- **Key functions:** `createAdCampaign`, `claimPayment`, `requestAndApproveWithdrawal`, `addAdmin`, `removeAdmin`,
  `setBankContractAddress`, `setCostPerClick`, `setCostPerImpression`, `getAdCampaignByCode`
- **Access roles:** Anyone can create a campaign (with ETH); admins/owner claim payments; advertiser or admin can
  withdraw remainder
...
> **As an** advertiser, **I want to** create an on-chain ad campaign by depositing ETH, **so that** payments can be
> claimed against my budget in a trustless way.
...
> **As an** admin, **I want to** claim payment from a campaign's budget based on reported clicks or impressions, **so
> that** the platform is compensated for traffic delivered.
...
> **As an** advertiser, **I want to** stop my campaign and withdraw the unspent ETH budget, **so that** I can recover
> funds if the campaign is no longer needed.
...
> **As an** owner, **I want to** set the global cost-per-click and cost-per-impression rates, **so that** all campaigns
> use the same pricing without per-campaign configuration.
...
> **As an** owner, **I want to** add or remove admin addresses, **so that** I can delegate campaign payment claims
> without giving full ownership.
