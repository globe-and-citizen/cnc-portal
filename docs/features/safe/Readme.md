...
3. Approve and execute the resulting Safe transaction according to the current threshold, as described in US-SAFE-006.
...
- [x] A user who is not a Safe owner cannot use these controls and sees why signer permission is required.
...
- The current state and its next step:
  ...
  - **Invalid:** the Safe has passed the transaction nonce; a new proposal is required if the action is still needed.
