...
All security requirements follow platform standards. See [Security Standards](../../platform/security.md) for details.

**Stats-Specific Requirements:**
...
All performance requirements follow platform standards. See [Performance Standards](../../platform/performance.md) for details.

**Stats-Specific Targets:**
...
Follow platform standards for scalability and reliability. See [Architecture](../../platform/architecture.md) for details.

**Stats-Specific Considerations:**
...
All testing requirements follow platform standards. See [Testing Strategy](../../platform/testing-strategy.md) for details.

**Stats-Specific Requirements:**

- Unit test coverage > 80%
- Comprehensive API documentation (OpenAPI/Swagger)
- Clear error messages for statistics endpoints
...
Using Zod schemas in `/backend/src/validation/schemas/stats.ts`:
...
```typescript
router.get('/overview', validate(overviewStatsSchema), getOverviewStats)
```
...
- User doesn't have access to requested team
...
2. Log detailed error to console with stack trace
3. Return user-friendly error message
4. Track errors with Sentry (if configured)
5. Don't expose sensitive information in error messages

**Frontend:**

1. Display error toast notification
2. Show inline error alerts for form validation
3. Provide retry mechanisms
4. Log errors to console for debugging
5. Clear authentication on 401 errors

---

## 8. Performance Optimization

The Statistics feature follows platform performance standards. See [Performance Standards](../../platform/performance.md) for general optimization strategies.

### 8.1 Stats-Specific Database Indexing

**Required Indexes:**

- `User.walletAddress` (unique index)
- `Team.ownerId` (foreign key index)
- `Claim.teamId`, `Claim.createdAt` (composite index)
- `Claim.userId`, `Claim.status` (composite index)
- `Wage.teamId`, `Wage.isActive` (composite index)
- `Expense.teamId`, `Expense.createdAt` (composite index)

### 8.2 Stats-Specific Response Time Targets
...
- `/contracts`: < 200ms
- `/actions`: < 200ms
- `/activity/recent`: < 500ms

### 8.3 Query Optimization Guidelines

- Use Prisma's aggregation functions for statistics calculations
- Implement pagination for large result sets (default: 20, max: 100)
- Use `select` to fetch only required fields
- Avoid N+1 queries with proper `include` usage
- Use `Promise.all()` for parallel independent queries

### 8.4 Caching Strategy (Future Enhancement)

**Cache Candidates:**

- Overview statistics: 5-minute TTL
- Team statistics: 10-minute TTL  
...
The Statistics feature follows platform security standards. See [Security Standards](../../platform/security.md) for comprehensive security requirements.

### 9.1 Stats-Specific Security

**Authentication & Authorization:**

- All endpoints require JWT authentication (Bearer token)
- Team-specific data filtered by ownership
- Platform administrators have full access to all statistics
- Team owners access only their team statistics
- No cross-team data access permitted

**Rate Limiting:**

- All `/api/stats/*` endpoints: 100,000 requests per 15 minutes per IP
...
- UUID format validation for `teamId` parameter
- Enum validation for `period` parameter: `'7d' | '30d' | '90d' | 'all'`
- Integer validation for pagination parameters (`page`, `limit`)
...
- Vue auto-escaping for XSS prevention
...
```bash
DATABASE_URL=postgresql://user:password@host:5432/cnc_portal
...
NUXT_PUBLIC_NETWORK_ALIAS=sepolia
```
...
4. Test statistics endpoints in staging
5. Deploy to production

**Rollback Plan:**

1. Revert to previous database snapshot
