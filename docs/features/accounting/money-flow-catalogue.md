...
| **Contra account**           | An account that sits in a class but carries the **opposite** side, reducing that class. `Deferred SHER Compensation` is a contra-equity account.      |
...
| **Deferred SHER Compensation**                            | Equity (**contra**) | **Debit**      | The offsetting debit of a SHER wage. Keeps share-based pay off the income statement — see [SHER compensation is equity, not an expense](#sher-compensation-is-equity-not-an-expense) |
...
### SHER compensation is equity, not an expense

When a member works for SHER, **nothing leaves the treasury** — the company issues its own
shares. So the whole lifecycle stays inside equity and **never reaches the income
statement**:

```
WORK EARNED   Dr Deferred SHER Compensation   (contra-equity, debit balance)
                 Cr SHERS To Be Issued            (equity, credit balance)

SHER ISSUED   Dr SHERS To Be Issued
                 Cr Investor Equity
```

This is the grant-date entry of a US-GAAP restricted-stock award: an equity account
created and exactly offset by a contra-equity account, with no P&L impact. CNC stops
there — it does **not** amortise the deferred balance into an expense over a vesting
period, because there is no vesting period: the work is already done when the entry is
booked. `Deferred SHER Compensation` is therefore a **permanent** debit balance.

Two consequences worth knowing when reading the statements:

- The **net effect of a SHER wage on total equity is nil** — the contra debit cancels the
  `Investor Equity` credit exactly. That is the correct outcome: no asset moved, so equity
  cannot move.
- The **cost is real but sits outside the result**. It shows up as dilution — in the SHER
  quantity column and in the on-chain share count — not as a charge. Reconcile
  `Σ Minted` against `Investor Equity` to see it.
...
- a **pending accrual** (SHER earned but not yet withdrawn, held in `SHERS To Be Issued`, UC-CASH-02) **floats at the current multiplier** — re-valued every time the multiplier moves, until it is withdrawn.

When a withdrawal settles an accrual, both legs carry the **withdrawal-date** value, so `SHERS To Be Issued` nets to zero with no revaluation account. A partly-withdrawn accrual is quantity-weighted: the withdrawn part frozen, the rest still floating.
...
flowchart LR
  ...
  sharesIssue[SHERS To Be Issued]:::equity
  deferredSher["Deferred SHER Compensation<br/>(contra-equity)"]:::equity
  ...
  deferredSher -->|"UC-CASH-02 · SHER wage promised (no P&L)"| sharesIssue
...
Payroll is recognised **when earned** (the claim), against a `Wage Payable` liability, then settled at the withdraw. Expense is recognised **only when paid**. The SHER part of a wage is **not** contributed equity at the claim: the contract only mints (`individualMint`) when the employee calls `withdraw()`. So the SHER promised sits in `SHERS To Be Issued` from claim to withdraw, and becomes `Investor Equity` **only at withdraw** — matching the on-chain `WithdrawToken` / `Minted` events. It never touches `Wage Payable`.

Note the asymmetry between the two parts of a wage: the **cash part is an expense** (real money leaves the treasury), while the **SHER part never touches the income statement** — see [SHER compensation is equity, not an expense](#sher-compensation-is-equity-not-an-expense).

```
CLAIM    Dr Payroll Expense              (cash + POL part)
            Cr Wage Payable
         Dr Deferred SHER Compensation   (SHER part — contra-equity, no P&L)
...
> **Default D.** A direct `individualMint` / `distributeMint` issues shares straight to equity — **Dr SHERS To Be Issued · Cr Investor Equity**, valued at the SHER rate frozen at the mint date. When the mint corresponds to earlier wage accruals it clears them out of `SHERS To Be Issued`; a mint with **no accrual behind it** debits `SHERS To Be Issued` into a **negative balance** — a known edge to reconcile (`Σ Minted` = on-chain supply, checked against the value in `Investor Equity`).
...
> **Claim vs withdraw timing.** Geor's wage claim (#9, 13 Mar) and withdrawal (#10, 15 Mar) are two days apart. The cash + POL owed sits in `Wage Payable` and the SHER promised sits in `SHERS To Be Issued` until the withdrawal settles both — see §5.3.
...
> **Ravi's direct mint (#17).** It has **no wage accrual behind it** (Ravi never claimed those 30 SHER), so the `Dr SHERS To Be Issued` has nothing to cancel: `SHERS To Be Issued` is pushed to a **−30 balance** and `Investor Equity` rises by 30. The books still balance (the entry is 30 = 30); this is the known unbacked-direct-mint edge from [§5.4](#54-sher-mints--three-paths-one-minted-event).
...
#17 Ravi direct mint  30 |
Solde (Dr, negative)  30 |   (unbacked direct mint — see #17)

Deferred SHER Compensation (Equity, contra)
Dr                       | Cr
#9  Geor claim (SHER) 10 |
Solde (Dr)            10 |   (never amortised — see §4)

Payroll Expense   (Dr) 40.8  — #9 (cash + POL part only)
...
| Deferred SHER Compensation | Equity (contra) |      10.00 |            |
...
> **No SHER line here, by design.** The 10 SHER Geor earned cost the company nothing in
> cash and never appear above: they were booked `Dr Deferred SHER Compensation ·
Cr SHERS To Be Issued`, both equity accounts. `Payroll Expense` therefore carries only
> the cash + POL part of the wage ($40.80). See
> [SHER compensation is equity, not an expense](#sher-compensation-is-equity-not-an-expense).
...
| SHERS To Be Issued (unbacked direct mint) |                        −30.00 |
| Deferred SHER compensation (contra)       |                        −10.00 |
| Retained earnings (net profit)            |                         13.97 |
| **Total equity**                          |                    **141.97** |
| **Assets = Liabilities + Equity**         | **141.97 = 0.00 + 141.97** ✅ |

> **The SHER wage nets to zero in equity.** Geor's 10 SHER added $10 to `Investor Equity`
> at withdraw and left a −$10 `Deferred SHER compensation` behind — no change to total
> equity, which is correct: no asset moved. Compared with booking it as an expense, the
> $10 simply sits on a different equity line (contra instead of retained earnings), so
> `Total equity` is unchanged either way.
...
- **Fees are cross-entity charges.** The team books $0.23 of `CNC Usage Fee Expense`, reducing team cash to $141.97 and team net income to $13.97. The CNC protocol separately books $0.23 of `Protocol Fee Revenue` and `Cash — FeeCollector` in §6.7.
- **Shares vs value.** `Investor Equity` ($168) counts capital raises ($100 + $10 + $10 + $8), the SHER paid as a wage ($10) and Ravi's direct mint ($30). Ravi's mint (**Default D**) issues 30 SHER straight to equity; because he never accrued them, the offsetting `Dr SHERS To Be Issued` has nothing to cancel and leaves that account at **−30** — the known unbacked-direct-mint edge. Still reconcile shares (`Σ Minted` = on-chain supply) against the value in `Investor Equity`.
- **SHER wages never reach the income statement.** The whole lifecycle is an equity movement: `Dr Deferred SHER Compensation · Cr SHERS To Be Issued` when the work is earned, then `Dr SHERS To Be Issued · Cr Investor Equity` when the shares are minted. The cost shows up as dilution, not as a charge — see [§4](#sher-compensation-is-equity-not-an-expense).
- **SHER is _issued_ at withdraw, not at claim.** From the claim until the withdrawal, the SHER part of a wage sits in `SHERS To Be Issued` — **floating at the current multiplier** while pending. Only at `withdraw()` does `individualMint` fire and the value move into `Investor Equity`, **frozen at the withdraw-date multiplier**. This matches the on-chain `WithdrawToken` / `Minted` events. The withdraw nets `SHERS To Be Issued` to $0; in a period where a claim is open without a matching withdrawal it carries the promised SHER, re-valued at the current multiplier. `Deferred SHER Compensation` is unaffected by any of this — it is booked once, at the claim, and stays.
