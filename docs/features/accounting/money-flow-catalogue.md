...
**Context:** The CNC accounting pipeline records each smart contract into the books one by one. Before we trust the resulting financial statements, this document does the opposite, top-down check: it lists **every way money can move** across the CNC contracts, so we can prove nothing the CNC actually does is missing from the books. Some money-moving contracts — `Tips`, `AdCampaignManager`, `Vesting` — are **not yet recorded** anywhere; this document flags them.

> **v1.1 (2026-06-14)** — Added after a contract review: §3.3 consolidated recap of SHER share issuance (**Default D** for plain mints), UC-CASH-04 restated as *share-based pay* at the SafeDepositRouter's fixed price, new **UC-CASH-06** (a single claim paid in several assets), §3.4 vesting split into two cases (cash vs SHER), and the reconciliation note "count SHER in shares, not in value".
...
| **Native vs ERC-20** | *Native* = the blockchain's own coin (ETH, or POL on Polygon). *ERC-20* = a token such as USDC or USDT. |
...
| 7 | **Vesting** | ✅ | ❌ | ✅ | ❌ **not indexed** | ⚠️ **Gap** |
| 8 | **Tips** | ✅ | ✅ | ❌ | ❌ **not indexed** | ⚠️ **Gap** |
| 9 | **AdCampaignManager** | ✅ | ✅ | ❌ | ❌ **not indexed** | ⚠️ **Gap** |
...
**Step 1 result:** 9 contracts move money. 5 are already covered by the accounting pipeline. **4 are not** (SafeDepositRouter, Vesting, Tips, AdCampaignManager) — these are the heart of the completeness check.
...
### 2.7 Vesting — deferred share/token grants *(not indexed)*

| Function | Asset | Direction | Caller | Event |
| -------- | ----- | --------- | ------ | ----- |
| `addVesting()` | ERC-20 | IN (lock in escrow) | team owner | `VestingCreated` |
| `release()` | ERC-20 | OUT → member | member | `TokensReleased` |
| `stopVesting()` | ERC-20 | OUT → member **+** OUT → owner (unvested) | team owner | `TokensReleased`, `UnvestedWithdrawn` |

### 2.8 Tips — tips *(not indexed)*

| Function | Asset | Direction | Caller | Event |
| -------- | ----- | --------- | ------ | ----- |
| `pushTip()` | native | OUT (paid directly) | anyone | `PushTip` |
| `sendTip()` | native | IN (credited to balance) | anyone | `SendTip` |
| `withdraw()` | native | OUT (withdraw credited balance) | beneficiary | `TipWithdrawal` |
| `receive()` | native | IN | anyone | — |

### 2.9 AdCampaignManager — ad campaigns *(not indexed)*

| Function | Asset | Direction | Caller | Event |
| -------- | ----- | --------- | ------ | ----- |
| `createAdCampaign()` | native | IN (budget) | advertiser | `AdCampaignCreated` |
| `claimPayment()` | native | OUT → Bank | admin/owner | `PaymentReleased` |
| `requestAndApproveWithdrawal()` | native | OUT → advertiser (unspent) **+** OUT → Bank (spent) | advertiser/admin/owner | `BudgetWithdrawn`, `PaymentReleasedOnWithdrawApproval` |
| `receive()` | native | IN | anyone | — |
...
*(Cr Cash at the grant = money leaving the treasury; or Owner Capital if the owner funds it from their own wallet.)*
...
- **UC-VEST-02** (*the recognition point*): `Dr Comp Expense · Cr Investor Equity` at the fixed price, **non-cash** — this is UC-CASH-04 triggered by `release`.
...
| UC-TIP-01/02/03 | Tips Expense / Tips Payable | 🔴 | 🔴 | 🔴 | 🔴 **Gap — not indexed** |
| UC-AD-01/02/03 | Ad Revenue / Deferred Ad Revenue | 🔴 | 🔴 | 🔴 | 🔴 **Gap — not indexed** |

### 5.1 Gaps to decide on

1. **Vesting** (UC-VEST-*) — deferred pay in tokens: no indexer, no account. **Decision needed:** index it + add `Vesting Escrow` / `Deferred Compensation`, or explicitly exclude it (and document why the cost of vested pay doesn't appear).
2. **Tips** (UC-TIP-*) — real native money flow with a credited balance (a liability). **Decision:** index it + add `Tips Payable`/`Tips Expense`, or exclude.
3. **AdCampaignManager** (UC-AD-*) — potential **revenue** (ad income) + a liability (revenue received in advance). **Decision:** index it; this is the only **non-fee revenue** stream → direct impact on the income statement if it's real.
4. **No LIABILITY class** in the chart of accounts — blocking for Tips, Ad, and accrual-based vesting.
5. **UC-BANK-03/05** (generic Bank transfers) — the purpose (expense? refund? transfer to another CNC contract?) can't be read from the event alone; needs a **purpose table** to decide the debit.

### 5.2 Reconciliation identities to add

On top of the spec's 8 identities, this catalogue adds:

- **TIPS_CASH:** Σ `sendTip` − Σ `withdraw` (Tips) = the on-chain `Tips Payable` balance.
- **AD_BUDGET:** Σ `createAdCampaign` − Σ (`claimPayment` + refunds) = the remaining on-chain budget of `AdCampaignManager`.
- **VESTING_ESCROW:** Σ `addVesting` − Σ `release` − Σ unvested-withdrawn = tokens still locked in escrow.
