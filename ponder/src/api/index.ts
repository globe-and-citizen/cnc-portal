...
async function fetchContractEvents(
  ...
  if (contractType === "Bank") {
    const [deposits, tokenDeposits, transfers, tokenTransfers, feesPaid, dividendTriggers] =
      await Promise.all([
        db.select().from(bankDeposit).where(eq(bankDeposit.contractAddress, address)),
        db.select().from(bankTokenDeposit).where(eq(bankTokenDeposit.contractAddress, address)),
        db.select().from(bankTransfer).where(eq(bankTransfer.contractAddress, address)),
        db.select().from(bankTokenTransfer).where(eq(bankTokenTransfer.contractAddress, address)),
        db.select().from(bankFeePaid).where(eq(bankFeePaid.contractAddress, address)),
        db
          .select()
          .from(bankDividendDistributionTriggered)
          .where(eq(bankDividendDistributionTriggered.contractAddress, address)),
      ]);
    return { deposits, tokenDeposits, transfers, tokenTransfers, feesPaid, dividendTriggers };
  }
  ...
  if (contractType === "Elections") {
    const [elections, votes, results] = await Promise.all([
      ...
      db.select().from(electionVote).where(eq(electionVote.contractAddress, address)),
      db.select().from(electionResult).where(eq(electionResult.contractAddress, address)),
    ]);
  ...
  if (contractType === "Proposals") {
    const [proposals, votes, tallies] = await Promise.all([
      ...
      db.select().from(proposalVote).where(eq(proposalVote.contractAddress, address)),
      db.select().from(proposalTally).where(eq(proposalTally.contractAddress, address)),
    ]);
  ...
  if (contractType === "BoardOfDirectors") {
    const [actions, approvals] = await Promise.all([
      db.select().from(boardAction).where(eq(boardAction.contractAddress, address)),
      db.select().from(boardApproval).where(eq(boardApproval.contractAddress, address)),
    ]);
  ...
  if (contractType === "InvestorV1") {
    const [mints, distributed, paid, failed] = await Promise.all([
      db.select().from(investorMint).where(eq(investorMint.contractAddress, address)),
      db
        .select()
        .from(investorDividendDistributed)
        .where(eq(investorDividendDistributed.contractAddress, address)),
      db.select().from(investorDividendPaid).where(eq(investorDividendPaid.contractAddress, address)),
      db
        .select()
  ...
  if (contractType === "CashRemunerationEIP712") {
    const [deposits, withdrawals, tokenWithdrawals, wageClaims] = await Promise.all([
      db
        .select()
  ...
  if (contractType === "ExpenseAccountEIP712") {
    const [deposits, tokenDeposits, transfers, tokenTransfers, approvals] = await Promise.all([
      db.select().from(expenseDeposit).where(eq(expenseDeposit.contractAddress, address)),
      db
        .select()
        .from(expenseTokenDeposit)
        .where(eq(expenseTokenDeposit.contractAddress, address)),
      db.select().from(expenseTransfer).where(eq(expenseTransfer.contractAddress, address)),
      db
        .select()
        .from(expenseTokenTransfer)
        .where(eq(expenseTokenTransfer.contractAddress, address)),
      db.select().from(expenseApproval).where(eq(expenseApproval.contractAddress, address)),
    ]);
...
app.get("/contracts/:address/events", async (c) => {
  ...
  return c.json(serializable({ contractAddress: address, contractType, teamAddress, events }));
});
...
app.get("/teams/:teamAddress/events", async (c) => {
  ...
  const eventsByType: Record<string, { contractAddress: string; events: Record<string, unknown[]> }> = {};
...
app.get("/teams/:teamAddress/timeline", async (c) => {
  ...
  const before = Number(c.req.query("before") ?? Math.floor(Date.now() / 1000) + 1);
  ...
  const [
    ...
    db
      .select()
      .from(bankDeposit)
      .where(and(inArray(bankDeposit.contractAddress, addresses), lt(bankDeposit.timestamp, before)))
    ...
    db
      ...
      .from(bankTransfer)
      .where(
        and(inArray(bankTransfer.contractAddress, addresses), lt(bankTransfer.timestamp, before)),
      )
    ...
    db
      ...
      .from(bankFeePaid)
      .where(
        and(inArray(bankFeePaid.contractAddress, addresses), lt(bankFeePaid.timestamp, before)),
      )
    ...
    db
      .select()
      .from(election)
      .where(and(inArray(election.contractAddress, addresses), lt(election.timestamp, before)))
    ...
    db
      .select()
      .from(proposal)
      .where(and(inArray(proposal.contractAddress, addresses), lt(proposal.timestamp, before)))
    ...
    db
      ...
      .from(boardAction)
      .where(
        and(inArray(boardAction.contractAddress, addresses), lt(boardAction.timestamp, before)),
      )
    ...
    db
      ...
      .from(safeDeposit)
      .where(
        and(inArray(safeDeposit.contractAddress, addresses), lt(safeDeposit.timestamp, before)),
      )
  ...
  type TimelineEvent = { eventType: string; timestamp: number; contractAddress: string; data: unknown };

  const tagged: TimelineEvent[] = [
    ...bankDeposits.map((r) => ({ eventType: "BankDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...bankTokenDeposits.map((r) => ({ eventType: "BankTokenDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...bankTransfers.map((r) => ({ eventType: "BankTransfer", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...bankTokenTransfers.map((r) => ({ eventType: "BankTokenTransfer", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...bankFeesPaid.map((r) => ({ eventType: "BankFeePaid", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...bankDividendTriggers.map((r) => ({ eventType: "BankDividendDistributionTriggered", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...elections.map((r) => ({ eventType: "ElectionCreated", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...electionVotes.map((r) => ({ eventType: "ElectionVote", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...electionResults.map((r) => ({ eventType: "ElectionResult", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...proposals.map((r) => ({ eventType: "ProposalCreated", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...proposalVotes.map((r) => ({ eventType: "ProposalVote", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...proposalTallies.map((r) => ({ eventType: "ProposalTally", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...boardActions.map((r) => ({ eventType: "BoardActionAdded", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...boardApprovals.map((r) => ({ eventType: "BoardApproval", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...investorMints.map((r) => ({ eventType: "InvestorMint", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...investorDividendsDistributed.map((r) => ({ eventType: "InvestorDividendDistributed", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...investorDividendsPaid.map((r) => ({ eventType: "InvestorDividendPaid", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...investorDividendsFailed.map((r) => ({ eventType: "InvestorDividendPaymentFailed", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...cashDeposits.map((r) => ({ eventType: "CashRemunerationDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...cashWithdrawals.map((r) => ({ eventType: "CashRemunerationWithdraw", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...cashTokenWithdrawals.map((r) => ({ eventType: "CashRemunerationWithdrawToken", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...cashWageClaims.map((r) => ({ eventType: "CashRemunerationWageClaim", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...safeDeposits.map((r) => ({ eventType: "SafeDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...expenseDeposits.map((r) => ({ eventType: "ExpenseDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...expenseTokenDeposits.map((r) => ({ eventType: "ExpenseTokenDeposit", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...expenseTransfers.map((r) => ({ eventType: "ExpenseTransfer", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...expenseTokenTransfers.map((r) => ({ eventType: "ExpenseTokenTransfer", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
    ...expenseApprovals.map((r) => ({ eventType: "ExpenseApproval", timestamp: r.timestamp, contractAddress: r.contractAddress, data: r })),
  ];

  tagged.sort((a, b) => b.timestamp - a.timestamp);
  const page = tagged.slice(0, limit);
  const nextBefore = page.length === limit ? page[page.length - 1]!.timestamp : null;
...
app.get("/users/:userAddress/activity", async (c) => {
  ...
  const [
    ...
    db
      .select()
      .from(bankDeposit)
      .where(and(eq(bankDeposit.depositor, userAddress), lt(bankDeposit.timestamp, before)))
      .orderBy(desc(bankDeposit.timestamp))
      .limit(limit),
    db
      .select()
      .from(bankTokenDeposit)
      .where(
        and(eq(bankTokenDeposit.depositor, userAddress), lt(bankTokenDeposit.timestamp, before)),
      )
      .orderBy(desc(bankTokenDeposit.timestamp))
      .limit(limit),
    db
      .select()
      .from(bankTransfer)
      .where(and(eq(bankTransfer.sender, userAddress), lt(bankTransfer.timestamp, before)))
      .orderBy(desc(bankTransfer.timestamp))
      .limit(limit),
    db
      .select()
      .from(bankTokenTransfer)
      .where(
        and(eq(bankTokenTransfer.sender, userAddress), lt(bankTokenTransfer.timestamp, before)),
      )
      .orderBy(desc(bankTokenTransfer.timestamp))
      .limit(limit),
    db
      .select()
      .from(electionVote)
      .where(and(eq(electionVote.voter, userAddress), lt(electionVote.timestamp, before)))
      .orderBy(desc(electionVote.timestamp))
      .limit(limit),
    db
      .select()
      .from(proposalVote)
      .where(and(eq(proposalVote.voter, userAddress), lt(proposalVote.timestamp, before)))
      .orderBy(desc(proposalVote.timestamp))
      .limit(limit),
    db
      .select()
      .from(boardApproval)
      .where(and(eq(boardApproval.approver, userAddress), lt(boardApproval.timestamp, before)))
      .orderBy(desc(boardApproval.timestamp))
      .limit(limit),
    db
      .select()
      .from(investorMint)
      .where(and(eq(investorMint.shareholder, userAddress), lt(investorMint.timestamp, before)))
      .orderBy(desc(investorMint.timestamp))
      .limit(limit),
    db
      .select()
    ...
    db
      .select()
      .from(safeDeposit)
      .where(and(eq(safeDeposit.depositor, userAddress), lt(safeDeposit.timestamp, before)))
      .orderBy(desc(safeDeposit.timestamp))
      .limit(limit),
    db
      .select()
      .from(expenseDeposit)
      .where(and(eq(expenseDeposit.depositor, userAddress), lt(expenseDeposit.timestamp, before)))
      .orderBy(desc(expenseDeposit.timestamp))
      .limit(limit),
    db
      .select()
    ...
    db
      ...
      .from(expenseTransfer)
      .where(
        and(eq(expenseTransfer.withdrawer, userAddress), lt(expenseTransfer.timestamp, before)),
      )
      .orderBy(desc(expenseTransfer.timestamp))
      .limit(limit),
    db
  ...
  type ActivityEvent = { eventType: string; timestamp: number; contractAddress: string; data: unknown };
...
app.get("/contracts/:address/board-actions", async (c) => {
  ...
  const actionsQuery = db
    ...
    .where(
      openOnly
        ? and(eq(boardAction.contractAddress, address), eq(boardAction.executed, false))
...
app.get("/contracts/:address/wage-claims", async (c) => {
  ...
  return c.json(serializable({ contractAddress: address, wageClaims: filtered }));
});
...
app.get("/contracts/:address/expense-approvals", async (c) => {
  ...
  return c.json(serializable({ contractAddress: address, expenseApprovals: filtered }));
});
